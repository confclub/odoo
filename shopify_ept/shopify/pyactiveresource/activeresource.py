...
class ActiveResource(six.with_metaclass(ResourceMeta, object)):
    ...
    def _find_every(cls, from_=None, **kwargs):
        ...
        if cls._plural == 'orders' and len(prefix_options):
        ...
        path_backup = path
        all_orders = []
        print('print total count ' + str(total_order_count))
        print('print total count ' + str(total_order_count))
        print('print total count ' + str(total_order_count))
        print('print total count ' + str(total_order_count))
        print('print total count ' + str(total_order_count))
        next = True
        while total_order_count > count:
            response = cls.connection.get(path, cls.headers)
            page_info_url = response.headers.get("Link")
            objs = cls.format.decode(response.body)
            all_orders += objs
            count = count + len(objs)
            if next and page_info_url:
                path = page_info_url.split("<")[1].split(">")[0]
                next = False
            elif count != total_order_count:
                next_page = page_info_url.split(",")[1]
                path = next_page.split("<")[1].split(">")[0]
            # count = count + len(objs)
            # path = path_backup + "&since_id=" + str(objs[-1].get('id'))
            # path = page_info_url.split(" ")[0]
        return cls._build_collection(all_orders, prefix_options, response.headers)
