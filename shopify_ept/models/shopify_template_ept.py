...
class ShopifyProductTemplateEpt(models.Model):
    ...
    def shopify_create_simple_product(self, product_name, variant_data, description, attribute_line_data=[]):
        ...
        uom = self.env['uom.uom'].search([('name', '=', variant_data['weight_unit'])])
        tax = self.env['account.tax'].search([("type_tax_use", "=", "sale")], limit=1)
        brand_name = False
        if variant_data.get('vendor'):
            brand_name = self.env['common.product.brand.ept'].search([('name', '=', variant_data.get('vendor'))]).id
            if not brand_name:
                brand_name = self.env['common.product.brand.ept'].create({
                    'name': variant_data['vendor'],
                }).id
        if sku or barcode:
            vals = {"name": product_name,
                    ...
                    "taxes_id": tax if variant_data['taxable'] == True else None,
    ...
    def shopify_sync_products(self, product_data_line_id, shopify_tmpl_id, instance, log_book_id,
        ...
        _logger.info("Process started for Product- %s || %s." % (template_data.get("id"),
        ...
        _logger.info("Process completed of Product- %s || %s." % (template_data.get("id"),
                                                                  template_data.get("title")))
