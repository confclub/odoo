...
from odoo.addons.gt_quickbook.api.exceptions import QuickbooksException
...
#     raise
from odoo.addons.gt_quickbook.api.service import OAuth1Service, OAuth2Service
from odoo.addons.gt_quickbook.api.session import OAuth1Session, OAuth2Session
...
class Oauth2SessionManager(AuthSessionManager):
    ...
    def __init__(self, **kwargs):
        # print "inittttttttttttttttttttttttt",kwargs 
    ...
    def start_session(self):
        if not self.started:
            if (self.client_id == '' or self.client_id == False):
                raise QuickbooksException("Client Id missing. Cannot create session.")

            if (self.client_secret == '' or self.client_secret == False):
                raise QuickbooksException("Client Secret missing. Cannot create session.")

            if (self.access_token == '' or self.access_token == False):
                raise QuickbooksException("Access Token missing. Cannot create session.")
    ...
    def get_auth_header(self):
        ...
        # print "HEADERRRRRRRRRRRRRRRRRRR",auth_header
    ...
    def token_request(self, payload, return_result=False):
        ...
        # print "RRRRRRRRRRRRRRRRRRRRRR",r.status_code ,r.text
    ...
    def get_access_tokens(self, auth_code, return_result=False):
        # print "get_access_tokenssssssssssssss",auth_code,return_result
        payload = {
            'code': auth_code,
            'redirect_uri': self.base_url,
            'grant_type': 'authorization_code'
        }
        # print "payloaddddddddddd",payload
